...
Matrix* projection(){
    ...
    m->matrix[0][0] = 1;
    m->matrix[0][1] = 0;
    m->matrix[0][2] = -cop->x/(cop->z-zp);
    m->matrix[0][3] = zp*cop->x/(cop->z-zp);

    m->matrix[1][0] = 0;
    m->matrix[1][1] = 1;
    m->matrix[1][2] = -cop->y/(cop->z-zp);
    m->matrix[1][3] = zp*cop->y/(cop->z-zp);


    m->matrix[2][0] = 0;
    m->matrix[2][1] = 0;
    m->matrix[2][2] = -zp/(cop->z-zp);
    m->matrix[2][3] = zp*(1+zp/(cop->z-zp));


    m->matrix[3][0]= 0;
    m->matrix[3][1] = 0;
    m->matrix[3][2] = -1/(cop->z-zp);
    m->matrix[3][3] = 1+zp/(cop->z-zp);
